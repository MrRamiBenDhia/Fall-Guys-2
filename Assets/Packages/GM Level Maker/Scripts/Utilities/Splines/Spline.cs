...
namespace GM.LevelMaker
{
	public class Spline : MonoBehaviour
	{
		...
		public void SubdivideCurve(int elt)
		{
			...
			points[elt * 3] = point;
			points[elt * 3 + 1] = point + (deriv * 0.2f);
			points[elt * 3 - 1] = point - (deriv * 0.2f);
		...
		public void RealignSpline()
		{
			SplineLoop loop = GetComponent<SplineLoop>();

			if (loop != null)
				RealignSpline(loop);
		}

		public void RealignSpline(SplineLoop _targetLoop, bool _fromEditSpline = false)
		{
			Vector3 offset = _targetLoop.GetControlPoint(0, true) - points[0];
