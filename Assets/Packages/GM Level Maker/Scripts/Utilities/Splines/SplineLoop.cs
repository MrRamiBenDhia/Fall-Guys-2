...
{
	public class SplineLoop : MonoBehaviour
	{

		public delegate void EditEvent();
		public EditEvent OnEdit;
		...
		// Format: p1, p2, p3...
		[SerializeField]
		private HandleConstraint[] constraints = new HandleConstraint[] {
		...
		public Vector3 GetPoint(float t, bool _local = false)
		{
			if (_local)
		...
		{
			if (index == GetControlPointCount() - 1)
				index = 0;
		...
		public Vector3 GetControlPoint(int index, bool _local = false)
		{
		...
		private Vector3? GetHandleRaw(int index, bool left)
		{
			...
			if (index == GetControlPointCount() - 1 && !left)
				index = 0;
		...
		private void SetHandleRaw(int index, bool left, Vector3 point, bool enforce_constraint = true)
		{
			if (index == 0 && left)
				index = GetControlPointCount() - 1;
			else if (index == GetControlPointCount() - 1 && !left)
				index = 0;
		...
		public void RemoveCurve(int elt)
		{
			...
			if (elt * 3 != points.Length)
			{
				ShiftArray(ref points, -3, elt * 3 - 1);
				ShiftArray(ref constraints, -1, elt);
			}

			Array.Resize(ref points, points.Length - 3);
			Array.Resize(ref constraints, constraints.Length - 1);
		...
		public void SubdivideCurve(int elt)
		{
			...
			Vector3 point = GetPointRaw(elt - 0.5f);
			Vector3 deriv = GetDirectionRaw(elt - 0.5f);
			...
			points[elt * 3] = point;
			points[elt * 3 + 1] = point + (deriv * 0.5f);
			points[elt * 3 - 1] = point - (deriv * 0.5f);
			constraints[elt] = HandleConstraint.Free;
		...
		public void RecenterSpline()
		{
			Vector3 averagePos = Vector3.zero;

			for (int point = 0; point < points.Length; point++)
			{
				averagePos += points[point];
			}

			averagePos /= points.Length;

			Vector3 offset = -averagePos;
